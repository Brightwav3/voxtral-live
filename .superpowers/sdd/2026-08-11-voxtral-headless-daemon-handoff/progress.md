...
Task 9 follow-up: default TTS model changed from `voxtral-mini-tts-latest` to the user-verified `voxtral-mini-tts-2603`, retaining `MISTRAL_TTS_MODEL` override (88/88 tests). Fresh push-to-talk plus `say`/`stop` was redacted and exited 0, but still emitted recoverable `invalid_input`; the streaming request shape needs comparison with the known-good direct request.
Task 9 follow-up: streamed `speech.audio.done` is now accepted as a terminal event and daemon session `text` is mapped to TTS `input` (89/89 tests). Direct live streaming and fresh daemon `say` on devices 13/19 completed with `assistant_final`, clean stop, and redacted capture; multi-turn and 20-second barge-in acceptance remain unverified.
